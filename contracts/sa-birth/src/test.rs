...
// Unit tests for the sa-birth contract using a simple mock GameHub.
...
use crate::{Error, SaBirthContract, SaBirthContractClient};
...
fn setup_test() -> (
    ...
    // Deploy sa-birth with admin and GameHub address
    let contract_id = env.register(SaBirthContract, (&admin, &hub_addr));
    let client = SaBirthContractClient::new(&env, &contract_id);

    // Register sa-birth as a whitelisted game (mock does nothing)
